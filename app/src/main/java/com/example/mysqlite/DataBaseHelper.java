...
public class DataBaseHelper {
    public class DatabaseHelper extends SQLiteOpenHelper {
        ...
        public void onCreate(SQLiteDatabase sqLiteDatabase) {
            sqLiteDatabase.execSQL(DataBaseTables.DatabaseTables.SQL_CREATE_TABLE_MOUNTAIN);
        }

        // This method is executed only if the database version has changed, e.g. from 1 to 2
        @Override
        public void onUpgrade(SQLiteDatabase sqLiteDatabase, int oldVersion, int newVersion) {
            sqLiteDatabase.execSQL(DataBaseTables.DatabaseTables.SQL_DELETE_TABLE_MOUNTAIN);
